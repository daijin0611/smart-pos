...
**领域模型命名规范**
  - VO：接口响应参数使用`xxxVO`命名，位置在`@root\pos-common\src\main\java\org\haut\common\domain\vo\**(对应模块)`，外层使用`JsonVO`进行包裹，`JsonVO`位置在`@root\pos-common\src\main\java\org\haut\common\domain\vo\JsonVO.java`
    - VO默认字段与实体类保持一致即可
  - DTO：接口请求非查询类参数使用`xxxDTO`命名，位置`@root\pos-common\src\main\java\org\haut\common\domain\dto\**(对应模块)`
    - PageDTO：分页查询结果DTO，将查询结果进行分页封装，位置`@root\pos-common\src\main\java\org\haut\common\domain\dto\PageDTO.java`
  - Entity：数据库实体类使用`xxxEntity`命名，位置`@root\pos-server\src\main\java\org\haut\server\stock\entity\**(对应模块)`
  - Query：接口请求查询类参数使用`xxxQuery`命名，位置`@root\pos-common\src\main\java\org\haut\common\domain\query\**(对应模块)`
  - 所有领域模型都应该还有`swagger3`注解，用于生成API文档，如：`@Schema`等

### 5. 业务相关信息
#### 5.1 订单
  - 优惠券使用规则：
    - 体验券：订单明细中含有体验券关联的项目，可以使用一张体验券抵扣该明细的金额。
    - 代金券：满足代金券使用条件后，代金券金额可以抵扣一部分订单总金额。
  - 会员资产使用规则：
    - 参与会员折扣的项目，订单明细实际单价更换折扣之后的价格。
    - 必须选择会员资产才能享受优惠。
    - 折扣计算公式：
      - 实际单价 = （标准价/会员价） * (1 - 会员折扣比例)。
      - 折扣基准需要根据会员资产来进行判断。
    - 非参与会员折扣的项目，订单明细实际单价保持不变。
    - 折扣相同的资产可以一起使用。
  - 手动优惠：
    - 明细优惠：直接指定实收价格。
    - 订单优惠：对订单总价直接进行优惠。
