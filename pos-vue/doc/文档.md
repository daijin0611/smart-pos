...
| `style`    | 代码格式调整（不影响功能，如空格、分号、缩进）。                       |
| `refactor` | 代码重构（既不修复 bug 也不新增功能）。                                |
| `test`     | 添加或修改测试代码。                                                   |
| `chore`    | 构建流程或辅助工具的变动（比如改变构建流程、或者增加依赖库、工具等）。 |
| `perf`     | 优化相关（比如提升性能、体验）                                         |
| `ci`       | 持续集成相关配置或脚本的修改（如 GitHub Actions、Jenkins）。           |
| `revert`   | 回滚之前的提交。                                                       |
...
- 支持 Google/Facebook 第三方登录
- 更新用户认证流程

feat!: 重构用户模块接口

BREAKING CHANGE: 用户 API 路径从 /api/users 改为 /api/v2/users
```
...
当我们 commit 提交信息时，就不能再随意写了，必须是 git commit -m 'fix: xxx' 符合类型的才可以，**需要注意的是类型的后面需要用英文的 `:`，并且冒号后面是需要`空一格`的，这个是不能省略的**
...
UI 组件库采用的 element-plus，因此需要集成 element-plus 插件！！！
...
项目中有诸多可以复用组件，所以我对其进行封装，减少冗余代码

### 3.1 Nav 三级导航组件封装

**组件地址**：`/src/components/ChildNav/index.vue`

**组件说明**：此组件用于三级导航，多个页面中使用到了此组件，所以对其进行封装，在其他页面引用是时，需传如一个数组`navList`，类型为`Array`，其中每一项为一个对象，包含三个属性：`label` `icon` `component`，以下为使用示例：
...
// label为nav按钮文本
// icon为要显示的图标
// component：为导航切换到的组件（注意：需使用markRaw()包裹转为非响应式数据）
