...
function getLodop(oOBJECT, oEMBED) {
  let strFontTag = "<br><font color='#FF00FF'>打印控件";
  let strLodopInstall = strFontTag + "未安装!点击这里<a href='install_lodop32.exe' target='_self'>执行安装</a>";
  let strLodopUpdate = strFontTag + "需要升级!点击这里<a href='install_lodop32.exe' target='_self'>执行升级</a>";
  let strLodop64Install = strFontTag + "未安装!点击这里<a href='install_lodop64.exe' target='_self'>执行安装</a>";
  let strLodop64Update = strFontTag + "需要升级!点击这里<a href='install_lodop64.exe' target='_self'>执行升级</a>";
  let strCLodopInstallA =
    "<br><font color='#FF00FF'>Web打印服务CLodop未安装启动，点击这里<a href='CLodop_Setup_for_Win32NT.exe' target='_self'>下载执行安装</a>";
  let strCLodopInstallB =
  ...
  try {
    ...
    if (needCLodop() || isLinuxX86 || isLinuxARM) {
      ...
      if (!LODOP) {
        ...
        // else if (isLinuxARM && LODOP.CVERSION < '7.1.0.5') strAlertMessage = strLodop7Update_ARM;
        // else if (CLODOP.CVERSION < '6.6.2.8') strAlertMessage = strCLodopUpdate;
      ...
      if (!LODOP || !LODOP.VERSION) {
        document.body.innerHTML =
          (isWinIE64 ? strLodop64Install : strLodopInstall) + strInstallOK + document.body.innerHTML;
        return LODOP;
      }
